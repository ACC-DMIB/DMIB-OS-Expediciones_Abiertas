...
Public Class frmAbiertas
    ...
    Public dtUnificarExpedicion As New DataTable
    ...
    Private Sub rellenarExpediciones()
        ...
        dtUnificarExpedicion = dtExpediciones
    ...
    Private Sub rellenarPedidosExpedicion()
        ...
        dtPedidos.Columns.Add("Estado OS")
        ...
        For Each row As DataRow In dtFinalAnterior.Rows
            ...
            If row("Nº Expedición") = numExp Then
                ...
                r("Estado OS") = returnEstadoOS(row("Pedido OS"))
    ...
    Private Sub rellenarListadoPedidosExpedicion()
        ...
        dtListadoPedidoExpedicion.Columns.Add("Máquina")
        dtListadoPedidoExpedicion.Columns.Add("Estado")

        Dim numExp As String = dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value

        Dim dt As DataTable = queries.contenidoExpedicion(numExp)

        For Each filaPedido As DataGridViewRow In dgvPedidos.Rows

            Dim pedido As String = filaPedido.Cells("Pedido OS").Value
            ...
            For Each row As DataRow In dt.Rows
                If row("codLineaPedido").ToString.Contains(pedido) Then
                    ...
                    Try
                        r("Máquina") = queries.obtenerMaquina(dtLinea(0)("number"))
                    Catch ex As Exception
                        r("Máquina") = ""
                    End Try

                    r("Estado") = dtLinea(0)("estado")
    ...
    Private Sub crearComboBox(dtPanel As DataTable)
        ...
        For Each c As DataColumn In dtPanel.Columns
            If c.ColumnName = "Sobre" Or c.ColumnName = "Caja Pequeña" Or c.ColumnName = "Caja Mediana" Or c.ColumnName = "Caja Grande" Or c.ColumnName = "Caja Cole" Or c.ColumnName = "Importe" Then
                Continue For
            End If
    ...
    Private Sub dgvExpediciones_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvExpediciones.CellDoubleClick
        ...
        'Si la celda donde hemos hecho el doble click no es la de Nº Expedición no valdría
        ''If dgvExpediciones.SelectedCells(0).ColumnIndex <> 0 And dgvExpediciones.SelectedCells(0).ColumnIndex <> 5 Then
        ''    Exit Sub
        ''End If

        ''If dgvExpediciones.SelectedCells(0).ColumnIndex = 5 Then
        ''    If dgvExpediciones.SelectedCells(0).Value = False Then
        ''        queries.urgenteTrue(dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value)
        ''    Else
        ''        queries.urgenteFalse(dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value)
        ''    End If
        ''    cargarGrid()
        ...
        If bool = True Then
            MsgBox("La expedición ya ha sido cerrada en otro equipo")
            cargarGrid()
            rellenarExpediciones()
            Exit Sub
        End If
        ...
        'Cerrar Expedicion
        'Dim respuesta As String = MsgBox("¿Está seguro de que quieres cerrar la expedición " & dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value & "?", vbYesNo)

        'Si queremos cerrar la expedicion
        'If respuesta = 6 Then
        If MessageBox.Show("¿Está seguro de que quieres cerrar la expedición " & dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value & "?", "ATENCIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            Dim idexpedicion As Integer = dgvExpediciones.Rows(dgvExpediciones.SelectedCells(0).RowIndex).Cells("Nº Expedición").Value

            Dim dt As DataTable = dgvLineasPedido.DataSource
            ' dt.Columns.Add("Estado OS")

            'Comprobamos los estados de cada pedido de OS de la expedicion, si es distinto de closed entonces no se cierra
            For Each row As DataRow In dt.Rows
                '  row("Estado OS") = returnEstadoOS(row("Pedido OS"))
                If row("Estado").ToString.ToUpper <> "CLOSED" Then
                    MsgBox("La línea de pedido " & row(0) & " tiene el estado " & row("Estado") & ", no se puede cerrar la expedición")
                    Exit Sub
                End If
            Next

            Dim b As Boolean = queries.cerrarExpedicion(idexpedicion)

            'Una vez que se ha cerrado la expedicion actualizamos los datos de nuevo 
            If b = True Then
                MsgBox("La expedición " & idexpedicion & " se ha cerrado correctamente")
            ...
            Else
                MsgBox("Error al cerrar la expedición. Llamad a informática")
            End If
    ...
    Private Function returnEstadoOS(pedidoOSS As String) As String
        getToken()
        ...
        Dim estado As String = pedidoOS.status
        Return estado
    End Function
